...
public class PersonMain {
    public static void main(String[] args) throws Exception {
        //클래스 객체 가져오기 (Class.forName() 메서드 방식)
        Class<Person> personClass = (Class<Person>)Class.forName("day06.api.reflectionEx.Person");

        //생성자 가져오기 - Person(String name, int age)
        Constructor<Person> constructor = personClass.getConstructor(String.class, int.class); //원하는 생성자 가져오기

        //가져온 생성자로 인스턴스 만들기
        Person p1 = constructor.newInstance("신세계", 25);
        //동적으로 메서드 가져와서 실행
        p1.getField();
        ...
        System.out.println(p1.total(3,2));
